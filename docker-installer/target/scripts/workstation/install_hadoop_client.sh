...
cd /home/vagrant/deploy/target/scripts/workstation
pwd
dos2unix addHadoopUser.sh addHadoopUser.sh
chmod +x addHadoopUser.sh
./addHadoopUser.sh $hostIp $clusterInfo $masterNodeIp
echo "install-hadoop-client: finished adding hadoop user"

dos2unix addYarnUser.sh addYarnUser.sh
chmod +x addYarnUser.sh
./addYarnUser.sh $hostIp $masterNodeIp
echo "install-hadoop-client: finished adding yarn user"


dos2unix addMapRedUser.sh addMapRedUser.sh
chmod +x addMapRedUser.sh
./addMapRedUser.sh
echo "install-hadoop-client: finished adding mapred user"

 
#Reference: https://hadoop.apache.org/docs/current/hadoop-project-dist/hadoop-common/ClusterSetup.html
sudo -u hadoop -H sh -c "mkdir -p /home/hadoop/Downloads"
sudo -u hadoop -H sh -c "wget 'http://www-us.apache.org/dist/hadoop/common/hadoop-2.7.6/hadoop-2.7.6.tar.gz' -O /home/hadoop/Downloads/hadoop.tar.gz"
sudo -u hadoop -H sh -c "mkdir -p /home/hadoop/hadoop"
cd /home/hadoop/hadoop
sudo tar -xvzf /home/hadoop/Downloads/hadoop.tar.gz --strip 1
echo "install-hadoop: finished extracting hadoop tar"

 
 #Add env variable in default .bashrc.
 sudo -u hadoop -H sh -c 'echo "export HADOOP_HOME=/home/hadoop/hadoop/" >> /home/hadoop/.bashrc'
 sudo -u hadoop -H sh -c 'echo "export JAVA_HOME=/usr/lib/jvm/java-8-oracle" >> /home/hadoop/hadoop/etc/hadoop/hadoop-env.sh'
 sudo -u hadoop -H sh -c 'echo "export HADOOP_SSH_OPTS='-o StrictHostKeyChecking=no'" >> /home/hadoop/hadoop/etc/hadoop/hadoop-env.sh'
